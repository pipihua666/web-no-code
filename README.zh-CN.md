...
按住 `Shift` 点击可以向选择中添加元素或移除已选元素，最多选择 8 个。未开启选择模式时，按住 `Option` / `Alt` 可以临时选择；同时按住 `Shift` 则可以临时添加或移除元素。最后选中的元素保持为主元素，CSS、图片和源码工具仍作用于它。

多选时，上下文标签会显示选择编号；当前主元素及其 Codex 上下文标签使用黄色高亮，其他附加元素使用绿色高亮和虚线轮廓。单选标签不显示编号。

顶部工具栏会用面包屑显示当前元素路径，并自动滚动以保持最后一级选中元素可见。点击其中的 `>` 分隔按钮可以查看同一父元素下的兄弟元素，无需返回预览区域即可切换选中元素。
...
| `Shift+点击`                 | 向选择中添加元素或移除已选元素    |
| `Option+Shift+点击`          | 临时添加或移除元素                |
